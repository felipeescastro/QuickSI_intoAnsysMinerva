...
# Find the last AEDT version installed in local PC
for env in environs:
...
if not os.path.exists(reports):
    os.makedirs(reports)
circuit = Circuit(specified_version=version_in_dot_format, projectname=os.path.join(perm_folder, project_name+".aedt"))
...
if not idealistic:
    ibis_path =  os.path.join(installPath ,"buflib","IBIS","u26a_800.ibs")
...
circuit.release_desktop( close_projects=True, close_desktop=True)
